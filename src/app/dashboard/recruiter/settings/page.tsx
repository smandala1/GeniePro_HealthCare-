        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="space-y-1.5">
              ...
              <input type="text" defaultValue={session?.user?.name ?? ""} className={inputCls} readOnly />
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-medium text-gray-600">Email Address</label>
              <input type="email" defaultValue={session?.user?.email ?? ""} className={inputCls} readOnly />
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <input type="password" value={passwords.current} onChange={(e) => setPasswords((p) => ({ ...p, current: e.target.value }))} placeholder="••••••••" className={inputCls} />
            </div>
            <div className="grid sm:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <label className="text-xs font-medium text-gray-600">New Password</label>
                <input type="password" value={passwords.next} onChange={(e) => setPasswords((p) => ({ ...p, next: e.target.value }))} placeholder="Min. 8 characters" className={inputCls} />
              </div>
              <div className="space-y-1.5">
                <label className="text-xs font-medium text-gray-600">Confirm Password</label>
                <input type="password" value={passwords.confirm} onChange={(e) => setPasswords((p) => ({ ...p, confirm: e.target.value }))} placeholder="Repeat password" className={inputCls} />
              </div>
            </div>
            <button
          ...
          <CardContent className="space-y-3">
            {(Object.keys(notifications) as Array<keyof typeof notifications>).map((key) => {
              const labels: Record<string, string> = {
                newApplications:    "New applications to your job postings",
                candidateMessages:  "Messages from candidates",
                weeklyReport:       "Weekly hiring activity report",
                platformUpdates:    "GeniePro platform updates",
              }
              return (
                <label key={key} className="flex items-center justify-between py-2 cursor-pointer">
                  ...
                  <button
                    ...
                    className={`relative inline-flex h-5 w-9 rounded-full transition-colors ${notifications[key] ? "bg-primary-500" : "bg-gray-200"}`}
                  >
                    <span className={`absolute top-0.5 left-0.5 h-4 w-4 rounded-full bg-white shadow transition-transform ${notifications[key] ? "translate-x-4" : "translate-x-0"}`} />
                  </button>
                </label>
              )
            })}
        ...
        <Card className="border-red-100">
          ...
          <CardContent>
            <p className="text-sm text-gray-500 mb-4">
              Deleting your account will remove all job postings and applicant data permanently.
            </p>
